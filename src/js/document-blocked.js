...
const setDisableWarningState = function(disabled) {
    const checkbox = qs$('#disableWarning');
    dom.attr(checkbox.closest('.checkbox'), 'disabled', disabled ? '' : null);
    dom.attr(checkbox, 'disabled', disabled ? '' : null);
};
...
const updateProceedButtonState = async function() {
    ...
    if ( masterPasswordConfigured !== true ) {
        ...
        setDisableWarningState(false);
        return;
    }
    const password = qs$('#masterPasswordInput').value;
    if ( password.length === 0 ) {
        proceedButton.disabled = true;
        setDisableWarningState(true);
        return;
    }
    const response = await messaging.send('documentBlocked', {
    ...
    setDisableWarningState(masterPasswordValidated !== true);
};
